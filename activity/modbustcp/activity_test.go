...
func TestReadHoldingRegisters(t *testing.T) {
	...
	tc.SetInput("numElements", 4)
	tc.SetInput("returnAs", "Float64")
	done, err := act.Eval(tc)
	assert.Nil(t, err)
	assert.True(t, done)
...
func TestWriteMultipleCoils(t *testing.T) {
	...
	tc.SetInput("address", 300)
	tc.SetInput("numElements", 2)
	tc.SetInput("data", []byte{0x03})
	tc.SetInput("returnAs", "Uint16")
	done, err := act.Eval(tc)
	assert.Nil(t, err)
	assert.True(t, done)
