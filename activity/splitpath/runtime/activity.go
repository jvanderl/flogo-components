	...
	if numparts > 8 {
		...
		for i := 0; i < 8; i++ {
			...
			switch i {
				...
				case 1 : context.SetOutput(part2, parts[position])
				case 2 : context.SetOutput(part3, parts[position])
				case 3 : context.SetOutput(part4, parts[position])
				case 4 : context.SetOutput(part5, parts[position])
				case 5 : context.SetOutput(part6, parts[position])
				case 6 : context.SetOutput(part7, parts[position])
				case 7 : context.SetOutput(part8, parts[position])
			}
		}
...
//			log.Debug("In for loop filling part: ", i)
			position := numparts - i
			switch position {
				case 1 : context.SetOutput(part1, parts[i])
				case 2 : context.SetOutput(part2, parts[i])
				case 3 : context.SetOutput(part3, parts[i])
				case 4 : context.SetOutput(part4, parts[i])
				case 5 : context.SetOutput(part5, parts[i])
				case 6 : context.SetOutput(part6, parts[i])
				case 7 : context.SetOutput(part7, parts[i])
				case 8 : context.SetOutput(part8, parts[i])
			}
		}
	}
