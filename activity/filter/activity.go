...
var log = logger.GetLogger("activity-jvanderl-filter")
...
func (a *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	if cxMinValue != "" {
		...
		if !ifInverse {
			...
			switch ivdatatype {
			case "int":
				if ifInput.(int) < ifMinValue.(int) {
					valueTooLow = true
				}
			case "uint":
				if ifInput.(uint) < ifMinValue.(uint) {
					valueTooLow = true
				}
			case "float32":
				if ifInput.(float32) < ifMinValue.(float32) {
					valueTooLow = true
				}
			}
		} else {
	...
	if cxMaxValue != "" {
		...
		if !ifInverse {
			...
			switch ivdatatype {
			case "int":
				if ifInput.(int) > ifMaxValue.(int) {
					valueTooHigh = true
				}
			case "uint":
				if ifInput.(uint) > ifMaxValue.(uint) {
					valueTooHigh = true
				}
			case "float32":
				if ifInput.(float32) > ifMaxValue.(float32) {
					valueTooHigh = true
				}
			}
		} else {
...
func validateValue(context activity.Context, element string, datatype string) interface{} {
	...
	switch datatype {
	case "int":
		dataOutput, err := strconv.ParseInt(dataInput, 10, strconv.IntSize)
		if err == nil {
			return int(dataOutput)
		}
	case "uint":
		dataOutput, err := strconv.ParseUint(dataInput, 10, strconv.IntSize)
		if err == nil {
			return uint(dataOutput)
		}
	case "float32":
		dataOutput, err := strconv.ParseFloat(dataInput, 32)
		if err == nil {
